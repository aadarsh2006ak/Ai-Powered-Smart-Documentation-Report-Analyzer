...
export default function RegisterPage() {
  ...
  const handleDemoLogin = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed to activate demo login.');
    } finally {
      ...
      <div className="w-full max-w-[420px] space-y-5 glass-panel p-6 sm:p-7 rounded-2xl border border-white/10 shadow-2xl relative overflow-hidden bg-gradient-to-b from-[#121A2E]/90 to-[#0A0E1A]/95">
        ...
        <div className="text-center space-y-1.5 relative z-10">
          ...
          <p className="text-xs text-slate-400">
            Start AI-driven document analysis and risk calculation
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3 relative z-10">
          ...
          <button
            ...
            className="w-full py-2.5 px-4 rounded-xl font-bold text-xs sm:text-sm text-white bg-gradient-to-r from-cyan-500 via-indigo-500 to-purple-600 hover:from-cyan-400 hover:to-purple-500 shadow-md shadow-cyan-500/20 flex items-center justify-center gap-2 transition-all duration-200 transform hover:scale-[1.01] disabled:opacity-50 mt-1"
          >
          ...
          >
            ...
            <span>{demoLoading ? 'Seeding Demo Data...' : '1-Click Instant Demo Login'}</span>
          </button>
        </form>
